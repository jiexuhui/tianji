  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="*胜负" prop="point1">
        <el-radio-group v-model="temp.point1" :disabled="temp.matchid==undefined">
          <el-radio-button
            :label="smatch.ltname+':胜'"
            @click.native.prevent="point1change(smatch.ltname+':胜')"
          >{{smatch.ltname}}胜</el-radio-button>
          <el-radio-button
            :label="smatch.rtname+':胜'"
            @click.native.prevent="point1change(smatch.rtname+':胜')"
          >{{smatch.rtname}}胜</el-radio-button>
        </el-radio-group>
      </el-form-item>
      ...
      <el-form-item label="*让分" prop="point3">
        <el-radio-group v-model="temp.point3">
          <el-radio-button
            v-for="item in silks3"
            :label="item"
            :key="item"
            @click.native.prevent="point3change(item)"
...
export default {
  ...
  data() {
    ...
    var checkPrice = (rule, value, callback) => {
      // callback(new Error(this.silknum));
      if (this.temp.price == 0 && this.silknum >= 4) {
        console.log("silknum>>>", this.silknum, "price>>", this.temp.price);
        callback(new Error("免费预测篇数本周已达上限"));
      } else {
        callback();
      }
    };
    return {
      ...
      rules: {
        ...
        content: [
          { required: true, message: "请输入基本面分析", trigger: "blur" }
        ],
        viewpoints: [
          { required: true, message: "请输入大神观点", trigger: "blur" }
        ]
      },
      smatch: {},
      silks2: [],
  ...
  methods: {
    ...
    selectChange(row) {
      ...
      (this.silks2 = []),
        (this.silks3 = []),
        matchList({ gameid: this.gameid, matchid: this.matchid }).then(
          response => {
            this.smatch = response.data[0][0];

            this.temp.matchid = this.smatch.id;
            console.log("smacth>>", this.smatch);
            this.smatch.silknames =
              this.smatch.silknames == null
                ? []
                : this.smatch.silknames.split(",");
            this.smatch.tsilks =
              this.smatch.tsilks == null ? [] : this.smatch.tsilks.split(",");
            this.smatch.tteams =
              this.smatch.tteams == null ? [] : this.smatch.tteams.split(",");
            for (const index in this.smatch.silknames) {
              const silk = {};
              const label =
                (this.smatch.tteams[index] == 1
                  ? this.smatch.ltname
                  : this.smatch.rtname) +
                ":" +
                this.smatch.silknames[index];
    ...
    getList() {
      ...
      matchList({ gameid: this.gameid, matchid: 0 }).then(response => {
        ...
        this.silknum = response.data[1][0].silknum;
        console.log("silknum:::>>", this.silknum);
        const silks2 = [];
        const silks3 = [];
        // this.listLoading = false;
      });
